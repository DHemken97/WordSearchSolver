namespace WordSearchSolver.Tests
{
    public class Tests
    {
        ...
        [Test]
        public void SolverShouldFindWords()
        {
            var sut = new Solver();
            sut.Board = GetTestBoard();
            sut.AllowSingleLetterWords = true;
            var results = sut.FindWords();
            var expectedWords = GetTestWords();
            Assert.GreaterOrEqual(results.Count,expectedWords.Count);
            var unfoundWords =
                expectedWords.Where(word => results.All(result => result.Word.ToUpper() != word.ToUpper()))
                    .ToList();
            Assert.False(unfoundWords.Any());
        }
        ...
        private List<string> GetTestWords()
        {
